...
@section('content')
    @php
        ...
        if (auth()->user()->role == 2) {
            ...
            $url_lampiran = 'report/' . $url_lampiran . '_report';
            $url_excel = 'report/' . $url_excel . '_report';
            $url_pdf = 'report/' . $url_pdf . '_report';
        }
    @endphp
    <main>
                        ...
                        <tbody>
                            ...
                            @foreach ($data['list'] as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-realgreen w-100 mb-2"
                                            href="{{ url($url_excel . '/' . $data['department']['id'] . '/' . $item->id) }}">Download
                                            Excel</a>
                                        <a class="btn btn-realred w-100 mb-2"
                                            href="{{ url($url_pdf . '/' . $data['department']['id'] . '/' . $item->id) }}">Download
                                            PDF</a>
                                    </td>
                                    <td>
                                        <a class="btn btn-realgreen w-100 mb-2"
